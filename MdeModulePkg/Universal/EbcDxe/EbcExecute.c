  ...
  )
{
  ...
  if ((Opcode & OPCODE_M_IMMDATA64) != 0) {
    ...
    if (!ADDR_IS_ALIGNED ((UINTN)Data64, sizeof (UINT16))) {
      EbcDebugSignalException (
  ...
  if (OPERAND1_INDIRECT (Operand)) {
    ...
    if (!ADDR_IS_ALIGNED ((UINTN)Addr, sizeof (UINT16))) {
      EbcDebugSignalException (
        EXCEPT_EBC_ALIGNMENT_CHECK,
        EXCEPTION_FLAG_FATAL,
        VmPtr
  ...
  )
{
  ...
  if (VmPtr->StackRetAddr == (UINT64)VmPtr->Gpr[0]) {
    ...
    // to it
    //
    if (!ADDR_IS_ALIGNED ((UINTN)VmPtr->Gpr[0], sizeof (UINT16))) {
      EbcDebugSignalException (
        EXCEPT_EBC_ALIGNMENT_CHECK,
        EXCEPTION_FLAG_FATAL,
        VmPtr
  ...
  )
{
  ...
  //
  if (ADDR_IS_ALIGNED (Addr, sizeof (UINT16))) {
    *(UINT16 *)Addr = Data;
  } else {
    //
    // Write as two bytes
  ...
  )
{
  ...
  // Do a simple write if aligned
  //
  if (ADDR_IS_ALIGNED (Addr, sizeof (UINT32))) {
    *(UINT32 *)Addr = Data;
  } else {
    //
    // Write as two words
  ...
  )
{
  ...
  // Do a simple write if aligned
  //
  if (ADDR_IS_ALIGNED (Addr, sizeof (UINT64))) {
    *(UINT64 *)Addr = Data;
  } else {
    //
  ...
  )
{
  ...
  // Do a simple write if aligned
  //
  if (ADDR_IS_ALIGNED (Addr, sizeof (UINTN))) {
    *(UINTN *)Addr = Data;
  } else {
  ...
  )
{
  ...
  //
  if (ADDR_IS_ALIGNED ((UINTN)VmPtr->Ip + Offset, sizeof (INT16))) {
  ...
  )
{
  ...
  // Read direct if aligned
  //
  if (ADDR_IS_ALIGNED ((UINTN)VmPtr->Ip + Offset, sizeof (UINT32))) {
  ...
  )
{
  ...
  // Read direct if aligned
  //
  if (ADDR_IS_ALIGNED ((UINTN)VmPtr->Ip + Offset, sizeof (UINT64))) {
  ...
  )
{
  ...
  // Read direct if aligned
  //
  if (ADDR_IS_ALIGNED ((UINTN)VmPtr->Ip + Offset, sizeof (UINT16))) {
