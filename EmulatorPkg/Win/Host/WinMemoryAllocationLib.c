  ...
  )
{
  ...
  DestSize = EFI_PAGES_TO_SIZE (Pages);
  Buffer   = VirtualAlloc (NULL, (SIZE_T)DestSize, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
  if (Buffer == NULL) {
  ...
  )
{
  ...
  DestSize = EFI_PAGES_TO_SIZE (Pages);
  Success  = VirtualFree ((VOID *)(UINTN)Memory, (SIZE_T)DestSize, MEM_DECOMMIT);
  if (!Success) {
