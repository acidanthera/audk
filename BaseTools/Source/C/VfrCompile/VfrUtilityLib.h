...
public:
  ...
  Pack (
    IN UINT32,
    IN UINT8,
    IN CHAR8   *Identifier = NULL,
    IN UINT32  Number      = DEFAULT_PACK_ALIGN
    );
...
private:
  ...
  VOID
  MarkVarStoreIdUnused (
    IN EFI_VARSTORE_ID
    );

  EFI_VARSTORE_ID
  CheckGuidField (
    IN SVfrVarStorageNode *,
    IN EFI_GUID *,
    IN BOOLEAN *,
...
public:
  ...
  EFI_VFR_RETURN_CODE
  AddBufferVarStoreFieldInfo (
    IN EFI_VARSTORE_INFO *
    );

  EFI_VFR_RETURN_CODE
  GetBufferVarStoreFieldInfo (
    IN OUT EFI_VARSTORE_INFO *
    );

private:
  CVfrDataStorage (
                   IN CONST CVfrDataStorage &
                   );                                      // Prevent copy-construction
  CVfrDataStorage &
  operator= (
    IN CONST CVfrDataStorage &
...
public:
  ...
  GetQuestionId (
    ...
    OUT EFI_QUESTION_ID &,
    OUT UINT32 &,
    OUT EFI_QUESION_TYPE  *QType = NULL
    );
