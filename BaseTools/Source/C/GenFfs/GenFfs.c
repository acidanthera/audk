...
--*/
{
  ...
  for (Index = 0; Index < InputFileNum; Index++) {
    ...
    DebugMsg (
      ...
      9,
      "Input section files",
      "the input section name is %s and the size is %u bytes",
      InputFileName[Index],
      (unsigned)FileSize
      );
    ...
    if ((InputFileAlign[Index] != 0) && (((Size + HeaderSize) % InputFileAlign[Index]) != 0)) {
      Offset = (Size + (UINT32)sizeof (EFI_COMMON_SECTION_HEADER) + HeaderSize + InputFileAlign[Index] - 1) & ~(InputFileAlign[Index] - 1);
      ...
      DebugMsg (
        ...
        0,
        9,
        "Pad raw section for section data alignment",
        "Pad Raw section size is %u",
        (unsigned)Offset
        );
...
--*/
{
  ...
  VerboseMsg (
    "FFS File Guid is %08X-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X",
