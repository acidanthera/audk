...
--*/
{
  ...
  DebugMsg (
    ...
    9,
    "comprss file size",
    "the original section size is %d bytes and the compressed section size is %u bytes",
...
--*/
{
  ...
  if (SectionName == NULL) {
    ...
    VerboseMsg (
      "Vendor Guid is %08X-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X",
