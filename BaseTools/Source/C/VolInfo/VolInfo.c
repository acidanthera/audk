...
{
  if (NewPath == NULL || AppendPath == NULL) {
    return EFI_INVALID_PARAMETER;
  }

  if (DefaultPath == NULL || DefaultPath[0] == '\0') {
    if (snprintf (NewPath, NewPathLen, "%s", AppendPath) >= (int)NewPathLen) {
      return EFI_ABORTED;
    }
  ...
  if (needQuote) {
    if (snprintf (NewPath, NewPathLen, "\"%s%c%s\"", DefaultPath, NATIVE_PATH_SEP, AppendPath) >= (int)NewPathLen) {
      return EFI_ABORTED;
    }
  } else {
    if (snprintf (NewPath, NewPathLen, "%s%c%s", DefaultPath, NATIVE_PATH_SEP, AppendPath) >= (int)NewPathLen) {
